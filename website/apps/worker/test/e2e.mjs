...
await s.open
check(await until(() => s.state?.phase === 'playing'), 'solo 自动开局')
...
check(await until(() => s.state.guesses.length === 1), '猜测被判定并广播')
...
check(await until(() => s.events.some((m) => m.event === 'error' && m.data.message.includes('太快'))), '冷却限流生效')
await sleep(1500)
s.send({ event: 'game:guess', data: { guess: '地'.repeat(len + 1) } })
check(await until(() => s.events.some((m) => m.event === 'error' && m.data.message.includes('个字'))), '字数校验')
s.send({ event: 'game:skip' })
check(await until(() => s.state.phase === 'reveal' && typeof s.state.answer === 'string'), '放弃后揭晓答案')
check(await until(() => s.state.phase === 'playing' && s.state.round === 2), '3 秒后 alarm 进入下一题')
s.ws.close()
...
await a.open
check(await until(() => a.state.phase === 'lobby' && a.state.players.length === 2), '私房等待 2 人')
...
check(await until(() => a.state.phase === 'playing' && b.state.phase === 'playing'), '房主开局，双方同步')
a.send({ event: 'game:guess', data: { guess: '人'.repeat(a.state.puzzle.wordLength) } })
check(await until(() => b.state.guesses.some((g) => g.name === '房主')), '对手的猜测对所有人可见')
a.ws.close()
b.ws.close()
...
const foundCode = (m) => m.events.find((e) => e.event === 'match:found')?.data.code
await until(() => ms.every(foundCode))
const found = ms.map(foundCode)
...
check(await until(() => rs[0].state?.phase === 'playing' && rs[0].state.players.length === 4), '全员到齐自动开局')
...
await until(() => two[0].events.some((e) => e.event === 'match:waiting' && e.data.count === 2))
